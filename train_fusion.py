...
# 导入损失函数
from loss import MultiTargetLoss
# 导入数据集
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, (ir_images, vi_images) in pbar:
            ...
            if args.fusion_type == "attn" and model.debug:
                ...
                total_loss, pix_loss, gra_loss, mean_loss = criterion(fused, ir_images, vi_images, aux["att_deep"])
            else:
...
def main():
    ...
    # 学习率与正则（分组）
    parser.add_argument("--lr_fuse", type=float, default=3e-4, help="融合层学习率")
    ...
    parser.add_argument("--w_gra", type=float, default=1.0)
    parser.add_argument("--w_mean", type=float, default=0.2)
    parser.add_argument("--w_ir", type=float, default=0.6)
    parser.add_argument("--w_vi", type=float, default=0.4)
    parser.add_argument("--attn_reg", type=float, default=0.0, help="注意力正则（0~1e-3 可尝试）")

    # 模型开关
    parser.add_argument("--fusion_type", type=str, default="attn", choices=["simple", "attn", "learnable"])
